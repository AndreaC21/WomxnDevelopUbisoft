...
bool Case::HasPlatform(Platform::ePosition direction)
{
...
void Level::genereLevel()
{
    ...
    this->SetObstacle(4,1, true);
    this->SetObstacle(4,4, true);
    this->SetObstacle(6,3, true);
...
m_Player()
{
    ...
    GenerateEnnemy();
    LoadUI();
    //m_JPS.Start(m_level, m_level.GetIndexByPosition(GetPlayer().GetPosition()), m_level.GetIndexByPosition(GetPortal().GetPosition()));
...
void GameLevel::UpdateCollisionWithPlayer()
{
    ...
    if (player.IsGhostMode()==false && player.IsColliding(GetPortal()))
    {
